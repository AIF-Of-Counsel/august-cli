# August Printed CLI Agent Guide

This directory is a generated `august-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
august-pp-cli doctor --json
august-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
august-pp-cli which "<capability>" --json
august-pp-cli <command> --help
```
...
```bash
august-pp-cli <command> --help
august-pp-cli <command> --dry-run --agent
```
