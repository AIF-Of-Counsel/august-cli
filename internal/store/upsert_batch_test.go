...
func TestUpsertBatch_TypedFailureDoesNotStrandContentsGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("contents count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "folders_id")
	}
}
...
// TestUpsertBatch_PopulatesProjectSearchTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed project_search table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesProjectSearchTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("project_search", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "project_search").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "project_search")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count project_search: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("project_search count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_SetsProjectSearchParentID verifies that dependent-resource
...
func TestUpsertBatch_SetsProjectSearchParentID(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	var matchedA int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE parent_id = ?`, "project_search")
...
// TestUpsertBatch_PopulatesQueriesFilesTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed queries_files table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesQueriesFilesTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "queries_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "queries_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "queries_id": "test-parent-001"}`),
	}
	if _, _, err := s.UpsertBatch("queries_files", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "queries_files").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "queries_files")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count queries_files: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("queries_files count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandQueriesFilesGeneric exercises
// the savepoint isolation around the typed-table dispatch. The fixture omits
// the NOT NULL parent FK column so the typed insert fails; the savepoint
// rolls back only the typed projection. The generic resources row inserted
// just before must survive. Regression for issue #1392, where a single
// outer transaction caused typed-table failures to cascade and silently
// discard every successfully fetched API row.
func TestUpsertBatch_TypedFailureDoesNotStrandQueriesFilesGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// Items deliberately omit "queries_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
		json.RawMessage(`{"id": "orphan-001"}`),
		json.RawMessage(`{"id": "orphan-002"}`),
	...
	stored, extractFailures, err := s.UpsertBatch("queries_files", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v (typed-table failure must not propagate)", err)
	}
	if stored != len(items) {
	...
	if typed != 0 {
		t.Fatalf("queries_files count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "queries_id")
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandResultGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("result count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "queries_id")
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandStatusGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("status count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "queries_id")
	}
}
