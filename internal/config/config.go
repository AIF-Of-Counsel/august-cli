...
type Config struct {
	...
	AugustAuthorization string            `toml:"authorization"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://app.august.law/api/v1",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("AUGUST_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "august-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("AUGUST_AUTHORIZATION"); v != "" {
		cfg.AugustAuthorization = v
		cfg.markEnvOverride("AugustAuthorization")
		cfg.AuthSource = "env:AUGUST_AUTHORIZATION"
	}
	...
	if cfg.AuthSource == "" && cfg.AugustAuthorization != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("AUGUST_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.AugustAuthorization != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:AUGUST_AUTHORIZATION"
		}
		return "Bearer " + c.AugustAuthorization
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientID")
	c.updateFileConfigField("ClientSecret")
	c.AugustAuthorization = ""
	delete(c.envOverrides, "AugustAuthorization")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AugustAuthorization"] {
			out.AugustAuthorization = c.fileConfig.AugustAuthorization
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AugustAuthorization":
		c.fileConfig.AugustAuthorization = c.AugustAuthorization
	}
}
