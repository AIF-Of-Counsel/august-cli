...
In local mode: searches locally synced data only.`,
  ...
  august-pp-cli search "error timeout"

  # Force local search only
  august-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  august-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  august-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'august-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "search_resource":
				results, err = db.SearchSearchResource(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchSearchResource(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search search_resource failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
