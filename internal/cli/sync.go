...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "project_search":
		return db.UpsertProjectSearch(data)
	case "members":
		return db.UpsertMembers(data)
	case "queries":
		return db.UpsertQueries(data)
	case "queries_files":
		return db.UpsertQueriesFiles(data)
	case "result":
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "project_search", ParentTable: "projects", ParentIDParam: "project_id", PathTemplate: "/api/v1/projects/{project_id}/search", KeyField: "", PathParams: []dependentPathParamDef{
