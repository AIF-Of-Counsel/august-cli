...
Resource scoping:
  ...
  august-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  august-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  august-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  august-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  august-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/august-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "content_search":
		return db.UpsertContentSearch(data)
	case "content":
		return db.UpsertContent(data)
	case "download_url":
		return db.UpsertDownloadUrl(data)
	case "folders":
		return db.UpsertFolders(data)
	case "tree":
		return db.UpsertTree(data)
	case "pre-share-policies":
		return db.UpsertPreSharePolicies(data)
	case "revert":
		return db.UpsertRevert(data)
	case "projects":
		return db.UpsertProjects(data)
	case "projects_folders":
		return db.UpsertProjectsFolders(data)
	case "members":
		return db.UpsertMembers(data)
	case "search_resource":
		return db.UpsertSearchResource(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"pre-share-policies": "/pre-share-policies",
		"projects":           "/projects",
	}
