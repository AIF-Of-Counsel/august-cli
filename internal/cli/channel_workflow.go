...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  august-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  august-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"chats", "folders", "pre-share-policies", "projects"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  august-pp-cli workflow status

  # Show status as JSON
  august-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("august-pp-cli")
			}
