...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "august-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AUGUST_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AUGUST_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "august-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/august-pp-cli/feedback.jsonl.
When ` + "`AUGUST_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AUGUST_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  august-pp-cli feedback list
  august-pp-cli feedback list --limit 5
  august-pp-cli feedback list --json`,
