...
large datasets as it has no memory pressure.`,
  ...
  august-pp-cli export <resource> --format jsonl --output data.jsonl

  # Export with limit
  august-pp-cli export <resource> --format jsonl --limit 1000

  # Pipe to another tool
  august-pp-cli export <resource> --format jsonl | jq '.id'`,
		Args: cobra.MinimumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			validResources := map[string]bool{
				"chats":              true,
				"pre-share-policies": true,
				"projects":           true,
			}
